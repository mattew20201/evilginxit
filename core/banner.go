...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - -- Updated at: 2024.04.29 |  https://t.me/Aizen_blackbeard  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by EvilHoster Telegram (") + handleClr.Sprintf("@Aizen_blackbeard") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
